...
对于修复bug，功能调整来说，codex能完全胜任，如果从头开发一个完整的大型系统，codex还面临很多挑战，特别是容易失控，这时最重要是对边界进行明确的约束，确定技术栈，功能边界等，对于一个初始化项目，首先确认 `AGENTS.md` 和 `DESIGN.md` 两个文件：`AGENTS.md` 用于约束技术栈、工程规范和执行边界，`DESIGN.md` 用于约束配色、字体、组件风格等视觉规范。第二步是选择技术栈，要选择AI擅长使用的技术栈如react+typescript，而不是人类擅长的技术栈如Java。
